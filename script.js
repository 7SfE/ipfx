...
function initApp() {
    ...
    document.getElementById('detectBrandsBtn').addEventListener('click', () => {
        const detected = batchDetectDeviceBrands();
        if (detected > 0) {
            showToast(`已为 ${detected} 个设备添加品牌信息`, 'success');
        }
    });
    ...
    if (manufacturer && manufacturer !== '') {
        ...
        for (const brandKey in deviceBrands) {
            if (upperManufacturer.includes(brandKey.toUpperCase())) {
                ...
                if ((brandInfo.type === "路由器/交换机" && (type === "路由器" || type === "交换机")) ||
                    (brandInfo.type === "打印机" && type === "打印机") ||
                    brandInfo.type === "通用") {
                    return {
...
function updateDeviceNotesWithBrand(device, brandInfo) {
    ...
    // 检查备注中是否已包含品牌信息
    if (device.notes && device.notes.includes(brandText)) {
        return; // 已存在，不重复添加
    }
 ...
 */
function batchDetectDeviceBrands(deviceList = null) {
    const devicesToProcess = deviceList || devices;
    let detectedCount = 0;
    
    devicesToProcess.forEach(device => {
        // 对路由器、交换机和打印机进行品牌识别
        if (device.type === '路由器' || device.type === '交换机' || device.type === '打印机') {
    ...
    if (detectedCount > 0) {
        ...
        mergeDataAndUpdate();
        saveToLocalStorage();
        showToast(`成功识别并添加了 ${detectedCount} 个设备品牌信息`, 'success');
    } else {
        showToast('未检测到新的品牌信息', 'info');
    }
...
function handleDeviceCSVUpload(content) {
    ...
    for (let i = 1; i < lines.length; i++) {
        ...
        device.isDomestic = isDomesticDevice(device.manufacturer, device.name);
    ...
    if (newDevices.length > 0) {
        ...
        if (brandDetectedCount > 0) {
            message += `，${brandDetectedCount} 个设备品牌已识别`;
        }
        if (invalidCount > 0) {
...
function mergeDataAndUpdate() {
    ...
    devices.forEach(device => {
        ...
        // 品牌识别（如果设备类型是路由器、交换机或打印机）
        if (device.type === '路由器' || device.type === '交换机' || device.type === '打印机') {
            // 如果尚未有品牌信息，尝试识别
            if (!device.notes || !device.notes.includes('[品牌:')) {
                const brandInfo = detectDeviceBrand(device);
                if (brandInfo) {
                    updateDeviceNotesWithBrand(device, brandInfo);
                }
            ...
            deviceScans.forEach(scan => {
                if (scan.detail) {
                    // 检查扫描详情中是否包含品牌关键词
                    const detailUpper = scan.detail.toUpperCase();
                    for (const brandKey in deviceBrands) {
                        if (detailUpper.includes(brandKey.toUpperCase())) {
                            // 如果备注中还没有这个品牌信息，添加
                            const brandText = `[品牌:${deviceBrands[brandKey].name}`;
                            if (!device.notes || !device.notes.includes(brandText)) {
                                const brandInfo = {
                                    brand: deviceBrands[brandKey].name,
                                    brandType: deviceBrands[brandKey].type,
                                    category: deviceBrands[brandKey].category,
                                    source: 'scan'
                                };
                                updateDeviceNotesWithBrand(device, brandInfo);
                            }
...
// 判断是否为国产设备
function isDomesticDevice(manufacturer, name) {
    const upperName = (name || '').toUpperCase();
...
function saveDeviceEdit() {
    ...
    // 如果是路由器/交换机/打印机，自动识别品牌
    if ((device.type === '路由器' || device.type === '交换机' || device.type === '打印机') && 
        (!device.notes || !device.notes.includes('[品牌:'))) {
        const brandInfo = detectDeviceBrand(device);
        if (brandInfo) {
            updateDeviceNotesWithBrand(device, brandInfo);
        }
...
function addNewDevice() {
    ...
    // 如果是路由器/交换机/打印机，自动识别品牌
    if ((newDevice.type === '路由器' || newDevice.type === '交换机' || newDevice.type === '打印机') && 
        (!newDevice.notes || !newDevice.notes.includes('[品牌:'))) {
        const brandInfo = detectDeviceBrand(newDevice);
        if (brandInfo) {
            updateDeviceNotesWithBrand(newDevice, brandInfo);
        }
    }
    ...
    let message = `
        <div class="duplicate-alert">
            ...
            <ul style="margin-left: 20px; margin-bottom: 15px;">
                <li><strong>覆盖操作：</strong>只覆盖IP地址、MAC地址、开放服务、安全风险</li>
                <li><strong>保留字段：</strong>设备名称、设备类型和备注不会被覆盖</li>
                <li><strong>GXXC设备：</strong>自动设为计算机和国产</li>
                <li><strong>品牌识别：</strong>自动识别路由器/交换机/打印机品牌</li>
            </ul>
            <p>请选择批量处理方式：</p>
        </div>
    `;
...
function refreshData() {
    ...
    // 重新识别品牌
    batchDetectDeviceBrands();
    
    mergeDataAndUpdate();
    updateTimestamp();
    showToast('数据已刷新，所有设备风险已重新评估，品牌已重新识别', 'success');
}
        ...
        <div class="risk-report">
            ...
            <div class="device-info-section">
                ...
                <table class="info-table">
                    ...
                    <tr><td><strong>品牌信息：</strong></td><td>${extractBrandFromNotes(device.notes)}</td></tr>
                </table>
            </div>
